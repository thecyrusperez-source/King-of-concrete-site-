...
- **26 real job photos** in `assets/gallery/` wired into the service blocks (every service has a photo; Repair & Leveling shows the before/after pair) and the masonry gallery (below Contact) with descriptive alt text.
...
2. **Quote destinations** — texts go to `(561) 410-6277` (`QUOTE_PHONE`) and the email fallback goes to `qlynum@yahoo.com` (`QUOTE_EMAIL`); both live in the script at the bottom of `index.html`.
